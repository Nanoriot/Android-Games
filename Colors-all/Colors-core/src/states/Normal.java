...
public class Normal extends GameState{
	...
	private ArrayList<cubeB> cubes;
	
	private float alpha;
	private boolean tin;
	private boolean tout;

	public Normal(GameStateManager gsm) {
		super(gsm);
		...
		while(loop){
			for(int row = 0;row < 5;row++){
				for(int col = 0;col<5;col++){
					...
					cubes.add(new cubeB(on,row,col));
					on = false;
				}
			}
	...
	public void render() {
		...
		font3.draw(batch,"" + score,(Core.V_WIDTH - le)/2,620);
	...
	public void save(){
		Preferences prefs = Gdx.app.getPreferences("HighScores");
		if(prefs.getLong("normal") < score){
			prefs.putLong("normal", score);
		}
		prefs.flush();
	}
