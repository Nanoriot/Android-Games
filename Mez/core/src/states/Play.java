...
public class Play extends GameState{
	...
	public Play(GameStateManager gsm) {
		...
		font3.setColor(Color.CYAN);
		font4.setColor(Color.ORANGE);
		
		generator.dispose(); // don't forget to dispose to avoid memory leaks!
		
		cubes = new ArrayList<cubeG>();
		generateCubes();
		...
		while(loop){
			for(int row = 0;row < 4;row++){
				for(int col = 0;col<4;col++){
					...
					cubes.add(new cubeG(on,row,col));
					on = false;
				}
			}
			if(count > 3) loop = false;
		}
	...
	public void render() {
		...
		font2.draw(batch,"" + score,(Core.V_WIDTH - le)/2,620);
	...
	public void save(){
		Preferences prefs = Gdx.app.getPreferences("HighScores");
		if(prefs.getLong("easy") < score){
			prefs.putLong("easy", score);
		}
		prefs.flush();
	}
